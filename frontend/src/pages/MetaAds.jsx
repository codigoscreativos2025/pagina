...
export default function MetaAds() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const res = await api.get('/integrations/meta-ads/campaigns')
      if (res.data.success) setData(res.data)
      else setError(res.data.error || 'Error cargando datos')
    } catch (err) { setError('No se pudo conectar con Meta Ads') }
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-slate-500">Cargando métricas de Meta Ads...</div>

  if (error) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <p className="text-6xl mb-4">📊</p>
        <h2 className="text-xl font-bold text-slate-800 mb-2">Meta Ads no disponible</h2>
  ...
  const kpis = [
    { label: 'Alcance', value: fmt(s.reach), icon: '👁️', color: 'bg-blue-50 text-blue-700' },
    { label: 'Impresiones', value: fmt(s.impressions), icon: '📱', color: 'bg-purple-50 text-purple-700' },
    ...
    { label: 'Gasto Total', value: fmtMoney(s.spend), icon: '💸', color: 'bg-red-50 text-red-700' },
  ]

  const costPerLead = crmLeads > 0 && s.spend ? (Number(s.spend) / crmLeads).toFixed(2) : null
    ...
    <div className="min-h-screen bg-slate-50">
      <nav className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <h1 className="text-xl font-bold text-slate-800">📣 Meta Ads — Rendimiento</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200">
            ...
            <div className="text-3xl font-bold text-blue-600">{campaigns.filter(c => c.status === 'ACTIVE').length}</div>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="text-right px-3 py-3 font-semibold text-slate-600">Alcance</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {campaigns.map(c => (
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-3 py-3 text-right font-medium">{fmt(c.insights?.reach)}</td>
                    ...
                    <td className="px-5 py-3 text-right font-bold">{fmtMoney(c.insights?.spend)}</td>
                  </tr>
                ))}
                {campaigns.length === 0 && (
                  <tr><td colSpan="7" className="px-5 py-8 text-center text-slate-400">No se encontraron campañas</td></tr>
                )}
              </tbody>
            </table>
          </div>
